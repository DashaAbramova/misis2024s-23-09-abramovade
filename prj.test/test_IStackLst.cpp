...
TEST_CASE_TEMPLATE("[stacklstt] - Constructor", T, int, float, double,
  ...
  SUBCASE("Move constructor") {
    ...
    moveCtor.push(1);
    moveCtor.push(4);
    StackLst<T> moveCtor2(std::move(moveCtor));
  ...
  SUBCASE("Initializer list constructor") {
    StackLst<T> initListCtor = {1, 2, 6, 4};
...
TEST_CASE_TEMPLATE("[stacklstt] - Push, Pop and Top", T, int, float, double,
  ...
  SUBCASE("Top and Pop with empty stack") {
    StackLst<T> s;
    CHECK_THROWS_WITH(s.pop(), "StackLst is empty");
    CHECK_THROWS_WITH(s.top(), "StackLst is empty");
    s.push(1);
    CHECK(s.top() == 1);
    CHECK(!s.empty());
    s.pop();
...
TEST_CASE_TEMPLATE("[stacklstt] - Compare", T, int, float, double, long long) {
  SUBCASE("Compare with non-empty stack") {
    StackLst<T> c1{2, 3, 1, 4};
    StackLst<T> c2{2, 3, 1, 4};
    StackLst<T> c3{5, 1, 2, 8};
    CHECK(c1 == c2);
    CHECK(c1 != c3);
  }
...
TEST_CASE_TEMPLATE("[stackarrt] - Swap and merge", T, int, float, double,
                   long long) {
  SUBCASE("Swap") {
    StackLst<T> s1{2, 3, 1, 4};
    StackLst<T> s1Cp{2, 3, 1, 4};
    StackLst<T> s2{5, 1, 2, 8};
    StackLst<T> s2Cp{5, 1, 2, 8};
    StackLst<T> s3;
    s1.swap(s2);
    CHECK(s1 == s2Cp);
    CHECK(s2 == s1Cp);
  ...
  SUBCASE("Merge") {
    StackLst<T> s1{1, 2, 3, 4};
    StackLst<T> s2{5, 6, 7, 8};
    StackLst<T> merged{1, 2, 3, 4, 5, 6, 7, 8};
    s1.merge(s2);
    CHECK(s1 == merged);
  }
  SUBCASE("Merge with empty stack") {
    StackLst<T> s1{1, 2, 3, 4};
    StackLst<T> s1Copy{1, 2, 3, 4};
    StackLst<T> s2;
    s1.merge(s2);
    CHECK(s1 == s1Copy);

    StackLst<T> s3;
    StackLst<T> s4 = {5, 6, 7, 8};
    StackLst<T> s3merged = {5, 6, 7, 8};
    s3.merge(s4);
    CHECK(s3 == s3merged);
  }
}
...
TEST_CASE_TEMPLATE("[stacklstt] - Assignment", T, int, float, double,
  ...
  CHECK(s1 == s2);
  StackLst<T> s3;
  StackLst<T> s4 = {5, 1, 2, 8};
  s4 = s3;
  CHECK(s4.empty());
}
TEST_CASE_TEMPLATE("[stacklstt] - stream output", T, int, float, double,
                   long long) {
  StackLst<T> s{2, 3, 1, 4};
